...
sun = sphere(pos=vec(0, 0, 0), color=color.orange, radius=sun_radii * scale_sun, emissive=True)
...
sun.v = vec(0, 0, 0)
# mimic sun light
local_light(pos=vec(0, 0, 0), color=color.white)
...
ss_label = label(pos=sun.pos, text=simulationSpeedString(), xoffset=scene.width * 0.5 - ss_xoffset, yoffset=-scene.height * 0.5 + ss_yoffset, border=3, line=0)
...
def addPlanet(name, pos, planet_velocity, require_color, planet_color, radii, mass):
    print('init ' + name + ' (' + str(scale_planet) + 'x)')
    my_planet = sphere(pos=pos, radius=radii * scale_planet, material=textures.rough, make_trail=planet_trail)
    my_planet.mass = mass
    my_planet.v = planet_velocity
    my_planet.comet = False
    my_planet.sun = False
    my_planet.defaultRadii = radii
    my_planet.name = name
    my_planet.reject = False
    # set planet color if needed
    if require_color:
        my_planet.color = planet_color
        my_planet.trail_color = planet_color
    # planet label
    _label = label(pos=my_planet.pos, color=my_planet.color, linecolor=my_planet.color, text=my_planet.name, xoffset=0, yoffset=30, line=1, opacity=0.4)
    my_planet.label = _label
    my_planet.label.visible = planet_label
    planets.append(my_planet)
    return my_planet


# add planets to simulation
mercury = addPlanet('mercury', vec(0.387 * dE, 0, 0), vec(0, 0, -1.607 * vE), True, color.gray(0.7), 0.383 * earth_radii, 0.0553 * earth_mass)
venus = addPlanet('venus', vec(0.723 * dE, 0, 0), vec(0, 0, -1.174 * vE), True, vec(1, 1, 0.5), 0.97 * earth_radii, 0.815 * earth_mass)
earth = addPlanet('earth', vec(dE, 0, 0), vec(0, 0, -vE), False, None, earth_radii, earth_mass)
earth.material = textures.earth
mars = addPlanet('mars', vec(1.52 * dE, 0, 0), vec(0, 0, -0.802 * vE), True, color.red, 0.532 * earth_radii, 0.107 * earth_mass)
jupiter = addPlanet('jupiter', vec(-5.2 * dE, 0, 0), vec(0, 0, 0.434 * vE), False, color.orange, earth_radii * 11.209, 317.83 * earth_mass)
saturn = addPlanet('saturn', vec(0, 0, -9.58 * dE), vec(-0.323 * vE, 0, 0), True, color.yellow, earth_radii * 9.499, 95.16 * earth_mass)
uranus = addPlanet('uranus', vec(19.20 * dE, 0, 0), vec(0, 0, -0.228 * vE), True, color.cyan, earth_radii * 4.007, 14.54 * earth_mass)
neptune = addPlanet('neptune', vec(30.05 * dE, 0, 0), vec(0, 0, -0.182 * vE), True, color.blue, earth_radii * 3.883, 17.15 * earth_mass)
...
# calculate distance vec between two objects
def distanceVector(obj1, obj2):
    return vec(obj1.pos - obj2.pos)
...
def addComet(pos, comet_velocity, comet_color, radii, mass):
    global n_comet
    _comet = sphere(pos=pos, color=comet_color, radius=radii * scale_comet, make_trail=comet_trail, trail_color=comet_color)
    ...
    _comet.mass = mass
    _comet.v = comet_velocity
    # comet orbit
    _comet.stat = None
    _comet.perihelion = distanceFromSun(_comet)
    print('init ' + _comet.name + ' with mass ' + str(mass) + ', radii ' + str(radii) + ', velocity ' + str(comet_velocity) + ', origin ' + str(pos))
    _comet.index = len(comets)
    ...
    if n_comet > 18:
        ...
        _n_comet -= 19
    # comet stat
    _stat = label(pos=sun.pos, color=_comet.color, text=cometString(_comet), xoffset=x_offset, yoffset=260 - 6 * (1.2 + 4.4 * _n_comet), line=0, opacity=0.4)
    _comet.stat = _stat
...
def interaction(obj1, obj2):
    vec_diff = obj1.pos - obj2.pos
    accel = -G * obj2.mass * vec_diff.norm() / vec_diff.mag2
    return accel
...
print("comet scale: {0}x".format(str(scale_comet)))
addComet(vec(-au(6), 1.2e11, 10e8), vec(10e2, -12e2, -22e2), color.orange, 300., 5000.)
addComet(vec(au(2), 10e9, -11e8), vec(5e2, 5.3e3, 6e3), color.green, 200., 150.)
addComet(vec(-1e10, au(4), 3e7), vec(4e3, -10e4, 4e3), color.cyan, 125., 200.)
addComet(vec(au(1.1), 300, 300), vec(-3e3, 5e3, -9e3), color.blue, 350., 600.)
addComet(vec(5.2e8, -au(1.9), -1e7), vec(-12e3, 27e3, -9e3), color.red, 200., 100.)
addComet(vec(-au(5.18), 1e4, 6e5), vec(-10.5e2, -9e3, 3e3), color.magenta, 100., 187.)
...
def process(command):
    ...
    if m_addComet:
        r, mass = 100., 100.
        comet_v, comet_o = vec(1e3, 1e3, 1e3), vec(1e11, 1e11, 1e11)
        ...
        if oau and specify_o:
            comet_o = one_au * comet_o
        addComet(comet_o, comet_v, randomColor(), r, mass)
        return
    ...
    if m_speed:
        ...
        if target_comet:
            ...
            if m_speed_group_size == 4:
                ...
                target_comet.v = vec(target_comet.v.x * vx, target_comet.v.y * vy, target_comet.v.z * vz)
...
class ControlWindow(threading.Thread):
    ...
    def __init__(self, root=None):
        ...
        self.root = root
        self.root.geometry('600x30+0+' + str(int(scene.height)))
...
def adjustPrecision(precision_type):
    global dt, precision_rate
    if precision_type == 1:
        if dt <= precision_rate:
            return
        dt /= precision_rate
    elif precision_type == -1:
        dt *= precision_rate
    if dt < 1:
        dt = 1
...
def showOrbitState(show, sim_object):
    if show:
        return 'hide ' + sim_object + ' orbit'
    return 'show ' + sim_object + ' orbit'
        ...
        if running:
            ...
            for comet1 in comets:
                ...
                if out_of_bounds or comet1.toBeReject:
                    comet1.stat.text += ' (X)'
                    print('disable ' + comet1.name + ' simulation', end=' ')
                    if out_of_bounds:
                        print('(bounds limit)')
            ...
            for predot in predots:
                ...
                if not predot.display:
                    # assign graph for each predot
                    predot.display = graph(width=900, height=300, background=color.white, foreground=color.black, xtitle='t(yr)', ytitle=predot.ytitle)
                    try:
                        ...
                        pass
                    if predot.type == 'dt':
                        gdot = gdots(graph=predot.display, size=predot.size)
                        gdot.color = predot.color
                        gdot.object1, gdot.object2 = predot.object1, predot.object2
                        predot.gdot = gdot
                    elif predot.type == 'et':
                        gdot1 = gdots(graph=predot.display, size=predot.size)
                        gdot2 = gdots(graph=predot.display, size=predot.size)
                ...
                if predot.type == 'dt':
                    predot.gdot.plot(t / one_year, distance(predot.object1, predot.object2) / one_au)
                elif predot.type == 'vt':
                    predot.gdot.plot(t / one_year, predot.object.v.mag / 1000.)
                elif predot.type == 'at':
                    predot.gdot.plot(t / one_year, predot.object.a.mag, 0)
                elif predot.type == 'et':
                    predot.gdot1.plot(t / one_year, kinetic_energy(predot.object) / 1e6)
                    predot.gdot2.plot(t / one_year, potential_energy(predot.object))
            t += dt
...
class SimulationWindow(threading.Thread):
    ...
    def togglePlanetOrbit(self):
        ...
        for _planet in planets:
            _planet.make_trail = planet_trail
            if not planet_trail:
                _planet.clear_trail()
    ...
    def randomComet(self):
        ...
        if not any_direction_comet:
            ...
            pre_o = vec(ox, oy, oz)
            pre_v_mag = vec(vx * (10 ** ov), vy * (10 ** ov), vz * (10 ** ov)).mag
            to_sun = sun.pos - pre_o
            proper_v = vec(to_sun.x + random.uniform(-1, 1), to_sun.y + random.uniform(-1, 1), to_sun.z + random.uniform(-1, 1))
            proper_v *= pre_v_mag
    ...
    def startSimulation(self):
        st = threading.Thread(target=start_simulation, args=())
        st.start()
    ...
    def __init__(self, root=None):
        ...
        first_frame = LabelFrame(self.root, text='Main')
        first_frame.pack()

        self.startSimulationBtn = Button(first_frame, text='START SIMULATION', command=self.startSimulation)
        self.startSimulationBtn.pack(side=TOP)
...
SimulationWindow(tsw)
# open command window
control = threading.Thread(target=start_control_window, args=(tsw,))
control.start()
tsw.mainloop()
